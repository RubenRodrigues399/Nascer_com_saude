...
import { newbornService, UpdateChildDto, UpdateChildDto } from '@/app/services/recem-nascidos';
...
// ─── Página principal ─────────────────────────────────────────────────────────import { locationsService, Province, Municipality, Neighborhood } from '@/app/services/locations';
import { unityService, UnityRecord } from '@/app/services/unidades';
...
export default function RecemNascidosPage() {
 ...
 Normalizado para evitar quebras por valores nulos
  // Filtra por unidade
  useEffect(() => {
    if (!filterUnityId) { loadAll(); return; }
    setLoadingFilter(true);
                ...
                filteredRecords.map(record => {
                  ...
                  return (
                    <tr key={id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-bold ${(record.status === 'pendente' || !record.id) ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}`}>
